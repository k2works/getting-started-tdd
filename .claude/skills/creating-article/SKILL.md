...
description: 「テスト駆動開発から始めるプログラミング入門」という多言語 TDD 記事シリーズに、新しいプログラミング言語を一つ立ち上げたいときに使う。ユーザーが Kotlin・Zig・Nim・Gleam・ReScript・Idris2・F#・Haskell など具体的な言語名を挙げて、「追加して」「〈言語〉版を一式そろえて／作って」「記事を書き起こして」「他言語と同じ 12 章構成で執筆と実装を進めて」「outline に足して執筆計画を立てて」と頼む場合が対象。第 1 部だけ・骨子と環境だけといった一部先行、依存型など第 4 部の扱いを変える変種、既存言語の章の増補も含む。docs/article/{lang}/・apps/{lang}/・integration・Nix 環境・CI を新規で用意する意図があれば発火する。既に揃っている言語の単発バグ修正・設定変更・章順の並べ替えだけの依頼、インセプションデッキ作成のような単発タスクには使わない。
---
