...
章の終わりに以下の流れで進める。**必ず「章の解説」を入れてから KPT 振り返りに移る**こと。解説で章全体を俯瞰し、共通の土台を作ったうえでユーザー自身の学びを言語化してもらう。

1. 学んだ概念の要約
2. TODO リストの最終状態確認
3. **章の解説**（必須）：学んだ概念・TDD サイクル・設計判断の意図を体系的に解説する
4. **KPT 振り返りの実施**（必須）：解説の内容を踏まえて Keep / Problem / Try を引き出す
5. 次章の予告と接続
6. （希望があれば）追加の練習問題を提案

#### 章の解説の進め方

KPT 振り返りに入る前に、章で扱った内容を AI 側から体系的に解説する。ユーザーが体験した Red-Green-Refactor サイクルや実装上の判断を、教材の意図と TDD の理論にひも付けて整理し、振り返りの土台を作る。

解説は以下の観点で構成する。

- **TDD サイクルの再確認**：章で実践した Red → Green → Refactor の各フェーズで何を意図していたか
- **設計判断のポイント**：仮実装・三角測量・明白な実装などのテクニックを、どこでなぜ使ったか
- **書籍・原典からの引用**：Kent Beck『テスト駆動開発』など関連文献の該当箇所を簡潔に引用し、理論的背景を補強する
- **次章への橋渡し**：章で扱わなかった発展的な論点や、次章で深掘りするテーマを予告する

解説の分量はユーザーの理解度に合わせて調整する。詰まった箇所が多かった場合は丁寧に、テンポよく進んだ場合は要点だけ短くまとめる。解説後に「ここまでで質問はありますか？」と一度確認を入れ、疑問が残らない状態で KPT に進む。

#### KPT 振り返りの進め方

章の解説で全体像を共有したうえで、KPT フレームワークで振り返りを行う。ユーザーに以下の 3 観点を順に問いかけ、回答を整理してまとめる。
