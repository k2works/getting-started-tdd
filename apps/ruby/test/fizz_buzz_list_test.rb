...
class FizzBuzzListTest < Minitest::Test
  ...
  def test_select_typeでFizzBuzzListを返す
    ...
    result = list.select_type { |value| value.value == 'Fizz' }
  ...
  def test_reject_typeでFizzBuzzListを返す
    ...
    command = FizzBuzzListCommand.new(type, 15)
    list = command.execute
    result = list.reject_type { |value| value.value == 'Fizz' }
  ...
  def test_find_valueで条件に合う最初の要素を返す
    ...
    command = FizzBuzzListCommand.new(type, 15)
    list = command.execute
    result = list.find_value { |value| value.value == 'Buzz' }
  ...
  def test_any_matchで条件に合う要素が存在するか
    ...
    assert(list.any_match? { |value| value.value == 'FizzBuzz' })
    refute(list.any_match? { |value| value.value == 'Unknown' })
  end
  ...
  def test_all_matchで全要素が条件を満たすか
    ...
    assert(list.all_match? { |value| value.number.positive? })
    refute(list.all_match? { |value| value.value == 'Fizz' })
  end
end
