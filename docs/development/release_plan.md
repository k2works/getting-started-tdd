# リリース計画 - テスト駆動開発から始めるプログラミング入門

## 概要

本ドキュメントは、「テスト駆動開発から始めるプログラミング入門」記事シリーズのリリース計画を定義します。
...
| Phase 4 | 追加言語（計画外の拡張） | 3 言語 |
| **合計** | | **15 言語 + 統合解説** |

### スケジュール

- **開発期間**: 24 週間（6 ヶ月）+ Phase 4（追加拡張）
- **イテレーション**: 2 週間 × 12 イテレーション（+ IT13〜15）
- **リリース**: 4 段階リリース（Phase ごと）
...
### Phase 4: 追加言語（イテレーション 13-15）

Phase 3 完了後に追加された拡張スコープ。いずれも 4 エピソード（12 章）構成。

| ID | ユーザーストーリー | SP | BV | C | KA | RR | 優先度 |
|----|-------------------|----|----|---|----|----|--------|
| US-014 | Kotlin の TDD 入門記事の執筆と実装 | 13 | 中 | 中 | 中 | 低 | 低 |
| US-015 | Flix の TDD 入門記事の執筆と実装 | 13 | 低 | 高 | 高 | 低 | 低 |
| US-016 | Prolog の TDD 入門記事の執筆と実装 | 13 | 低 | 高 | 高 | 低 | 低 |
| **合計** | | **39** | | | | | |
...
gantt
    ...
    section Phase 4
    IT13 Kotlin            :done, a4-1, 2026-07-18, 1d
    IT14 Flix              :done, a4-2, 2026-07-18, 1d
    IT15 Prolog            :done, a4-3, 2026-07-21, 1d
    Release 4.0            :milestone, am4, 2026-09-25, 0d
```
...
- [x] 全言語のテストがパス
- [x] 統合解説の内容が全言語をカバー
- [x] GitHub Milestone Release 3.0 クローズ済み
- [x] **v3.0.0 リリース済み（2026-03-04）**

#### Release 4.0（Phase 4 完了）: 追加言語

**目標**: Kotlin、Flix、Prolog の記事と実装を公開

**含まれる機能**:

- 3 言語 × 12 章の記事
- 3 言語の TDD 実装コード（apps/kotlin、apps/flix、apps/prolog）
- Nix 開発環境（kotlin / flix / prolog）の追加と flake 登録
- 統合解説・全章リンク表への 3 言語の反映
...
- [x] apps/haskell/ プロジェクト初期化（Stack + HSpec + HLint）
- [x] 第 1 部（章 1-3）: TDD 基本サイクルの執筆と実装
- [x] 第 2 部（章 4-6）: 開発環境と自動化の執筆
- [x] 第 3 部（章 7-9）: 型クラスと代数的データ型の執筆と実装
- [x] 第 4 部（章 10-12）: 関数型プログラミング（高階関数、関数合成、モナド）
- [x] 多言語統合解説（12 言語横断比較、学習ロードマップ）
- [x] Release 3.0 準備
...
32. ~~Phase 4: Kotlin 第 1〜12 章の執筆・実装を完了~~ ✅
